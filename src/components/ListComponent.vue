      ...
      <v-expansion-panel v-for=" task in listaTareas" :key="task.id">
        ...
        <v-expansion-panel-text>
          <v-row>
            ...
            <v-col cols="12" sm="6" md="4">
              Created: {{ formatDate(task.at_created) }}
            </v-col>
            <v-col cols="12" sm="6" md="4">
              Updated: {{ formatDate(task.at_updated) }}
            </v-col>
          </v-row>
          <v-row>
            <v-col>
            ...
            <v-col>
              <v-btn v-if="task.completed === false" color="success" @click="completed(task.id)" class="mx-1 mb-2">
                <v-icon>mdi-check</v-icon>
                Complete
              </v-btn>
              <v-btn v-if="task.completed === true" color="secondary" @click="Open(task.id)" class="mx-1 mb-2">
...
export default {
  ...
  methods: {
    viewById(id) {
      ...
      axios.get('http://127.0.0.1:8000/api/Tareas/' + id, {
        ...
        .then((response) => {
          ...
          localStorage.completedView = response.data.id
        })
        .catch(error => {
          console.log(error)
        })
    ...
    editTask() {
      ...
      let completedView = localStorage.completedView
